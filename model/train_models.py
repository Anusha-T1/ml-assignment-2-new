"""
Vinho Verde wine quality -- training pipeline for six classifiers.

Reads the two raw UCI CSVs (red + white), merges them into a single frame with a
`wine_type` indicator, binarises the 0-10 sensory score into a "premium bottle"
...
def assemble_frame() -> pd.DataFrame:
    """Merge the red and white Vinho Verde tables into one labelled frame."""
    ...
    cellar = pd.concat([reds, whites], ignore_index=True)
    cellar = cellar.rename(columns={"ph": "pH"})   # keep the conventional casing
    cellar = cellar.drop_duplicates().reset_index(drop=True)
    cellar[TARGET] = (cellar["quality"] >= PREMIUM_CUTOFF).astype(int)
    cellar = cellar.drop(columns=["quality"])
    return cellar[FEATURE_ORDER + [TARGET]]


def build_classifiers() -> dict:
    """The six estimators under comparison, keyed by the label used everywhere else."""
    return {
        ...
        "Random Forest (Ensemble)": RandomForestClassifier(
            # left unweighted on purpose: re-weighting the bootstrap samples
            # cost this forest ~3 MCC points in testing
...
def main() -> None:
    ...
    cellar = assemble_frame()
    print(f"cellar assembled: {cellar.shape[0]} bottles x "
          f"{len(FEATURE_ORDER)} features")
    print(f"premium rate: {cellar[TARGET].mean():.3%}")

    X = cellar[FEATURE_ORDER]
    y = cellar[TARGET]
    ...
    # the held-out slice doubles as the CSV the Streamlit app consumes
    ...
    for label, estimator in build_classifiers().items():
        ...
        # compressed: the 450-tree forest is ~31 MB raw, ~5 MB compressed
